...
REGION=
```
Copy the region from task 2 table
```BigQuery output table``` copy it from Task 1 Table 
```cmd
...
Bucket_TASK_3=
```
Copy it from task 3 instructions
```cmd
Bucket_TASK_4=
```
Copy it from task 4 instructions
```cmd
...
```
Type Y when it ask for continue and click on the link to login in the cloud console
```cmd
